...
import com.a.EnGround.controller.GameController;
...
public class SentenceQuizSystemImpl implements QuizSystem{
	...
	private static final Logger logger = LoggerFactory.getLogger(SentenceQuizSystemImpl.class);
	@Autowired
	FindQuizSystem findQuizSystem;

	@Autowired
	public SentenceQuizSystemImpl(QuizRepository quizRepository, GameRoomRepository gameRoomRepository, ObjectMapper objectMapper, GameSessionRepository gameSessionRepository, GameRecordsRepository gameRecordsRepository, InGameQuizRepository inGameQuizRepository) {
	...
	public GameRecordsVO setGameRecordsAndReturn(int score, String roomId, String userId) {
	    ...
	    historyVO.setGameMode("sentence");
	    // 점수 업데이트
	...
	public List<InGameQuizVO>loadQuestion(GameRoomVO vo, String categoryNum, String table, String userId) throws JsonMappingException, JsonProcessingException {
		...
		if(vo != null)
		{
			...
			inGameQuizVO.setGameMode("sentence");
			inGameQuizVO.setRoomId(userId+"_sentence");
		...
		{
		gameRoomRepository.insertGameInform(userId+"_sentence", "sentence", "sentence");
		...
		{
			inGameQuizRepository.insertInGameQuiz(userId+"_sentence", "sentence", item.getQuizNo());
		}
