			...
			<div class="waitUser-body">
				<div class="waitUser-userboard">
            		<div class="wait-board-name">
                        ...
                        <input type="text" id="roomName" name="roomName" value="제목">
                    </div>
            		<div class="wait-board-name">
                        <label for="timer">타이머</label>
                        <input type="text" id="timer" name="timer" value="60">
                    </div>
            	<div class="wait-board-name">
            		<label for="room-cate">카테고리</label>
            		<select  id="category" name="cateType">
            			<c:forEach var="item" items="${category}">
            			<option value="${item.categoryName}" ${cateType eq "${item.categoryName}" ? 'selected' : ''}>${item.categoryName}</option>
            			</c:forEach>
            		</select>
...
<script type="module" src="<c:url value='/resources/js/lobbyStomp.js' />" defer></script>
